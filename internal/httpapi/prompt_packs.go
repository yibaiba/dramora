...
func (api *api) getShotPromptPackRecovery(w http.ResponseWriter, r *http.Request) {
	recovery, err := api.productionService.GetShotPromptPackRecovery(r.Context(), chi.URLParam(r, "shotId"))
	if err != nil {
		writeServiceError(w, err)
		return
	}
	writeJSON(w, http.StatusOK, Envelope{"prompt_pack_recovery": promptPackRecoveryDTO(recovery)})
}
